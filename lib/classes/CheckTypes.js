...
var CheckTypes = /*#__PURE__*/function (_SetTypes) {
  ...
  function CheckTypes(_value) {
    ...
    _defineProperty(_assertThisInitialized(_this), "middleware", function (schema) {
      var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {
        strict: Boolean,
        blockIfErrorOccur: Boolean,
        includeParamsValue: Boolean
      };
      return function (req, res, next) {
        // Set empty rules default options
        var defaults = {
          strict: true,
          blockIfErrorOccur: false,
          includeParamsValue: false
        };
        Object.keys(defaults).forEach(function (key) {
          if (options[key] === undefined) options[key] = defaults[key];
        }); // Schema type check

        if (!(schema instanceof _Schema["default"]) || schema == null) throw new Error("Schema must be a valid instance of Schema class."); // Options type checK
        ...
        if (typeof options.includeParamsValue !== "boolean") throw new Error("Option `includeParamsValue` must be boolean"); // Get value based on method type, params included

        var method = req.method;
        var value = method !== "GET" ? _objectSpread({}, req.body) : _objectSpread({}, req.query);
        if (options.includeParamsValue) value = _objectSpread(_objectSpread({}, value), req.params); //

        var check = new CheckTypes().setValue(value).setSchema(schema).check({
        ...
        if (check.errors.length === 0) {
          ...
          req.body = _objectSpread({}, check.value);
          return next();
        } else {
          if (options.blockIfErrorOccur) throw new Error("Errors on middleware validation process.");else {
            req.check = {
              isValid: false,
              errors: check.errors
            };
